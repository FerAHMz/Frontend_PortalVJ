...
const calendarDays = computed(() => {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    const dayTasks = tasks.value.filter(task => new Date(task.fecha_entrega).toDateString() === date.toDateString());
...
const fetchTasks = async () => {
  try {
    ...
    if (response.ok) {
      ...
      assignCourseColors(); // Assign colors after fetching tasks
    } else {
      tasks.value = [];
    }
  } catch (error) {
